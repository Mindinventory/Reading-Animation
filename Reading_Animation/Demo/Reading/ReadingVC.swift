...
final class ReadingVC: UIViewController {
    ...
    private var readingData = [ReadingPage(page: 1),
                               ReadingPage(page: 2),
                               ReadingPage(page: 3),
                               ReadingPage(page: 4),
                               ReadingPage(page: 5),
...
extension ReadingVC {
    ...
    private func configure() {
        ...
        CMainThread.asyncAfter(deadline: .now() + 0.2, execute: {
            ...
            self.animateViews(btnBack: self.btnBack, lbls: (self.lblReading, self.lblReadingDetails),
                              constraint: (self.constBtnBackTop, self.constImgReadingCenterX),
                              vcView: self.view, coll: (self.collReading, self.readingData))
        })
    }
}
